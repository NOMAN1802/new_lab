...
console.log('\n--- 1: 3 tests = 2000, referrer 10% waiver / 15% commission ---');
const s1 = computeTotals(items(800, 700, 500), 10, 15, 0);
check('gross', s1.grossAmount, 2000);
check('waiver (10% of gross)', s1.waiverAmount, 200);
...
const s2 = computeTotals(items(800, 700, 500), 10, 15, 1000);
check('due', s2.dueAmount, 800);
check('status', s2.paymentStatus, 'partial');

console.log('\n--- 3: settled in full ---');
const s3 = computeTotals(items(800, 700, 500), 10, 15, 1800);
check('due', s3.dueAmount, 0);
check('status', s3.paymentStatus, 'paid');

console.log('\n--- 4: walk-in, no referrer (0% / 0%) ---');
const s4 = computeTotals(items(1200), 0, 0, 0);
check('waiver', s4.waiverAmount, 0);
check('net equals gross', s4.netPayable, s4.grossAmount);
check('commission', s4.commissionAmount, 0);

console.log('\n--- 5: 100% waiver (camp / staff case) ---');
const s5 = computeTotals(items(500), 100, 10, 0);
...
const s6 = computeTotals(items(333.33, 333.33, 333.33), 12.5, 7.5, 0);
check('gross', s6.grossAmount, 999.99);
check('waiver rounded to paisa', s6.waiverAmount, 125);
check('net', s6.netPayable, 874.99);
check(
  'gross - waiver === net (no drift)',
  Math.round((s6.grossAmount - s6.waiverAmount) * 100) / 100,
  s6.netPayable
);

console.log('\n--- 7: overpayment cannot drive due negative ---');
const s7 = computeTotals(items(100), 0, 0, 150);
